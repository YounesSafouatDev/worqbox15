...
class ASProductTemplateExtend(models.Model):
    ...
    product_web_description = fields.Text('Description Web', translate=True)
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if self.env.context.get("brands",False):
            base_domain.append([('product_website_brand_id','in',[int(i) for i in self.env.context.get("brands",[])])])
            res.update({"base_domain":base_domain})
