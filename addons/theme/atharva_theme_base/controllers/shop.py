...
class AtharvaWebsiteSaleExtend(WebsiteSale):
    ...
    def _get_search_domain(self, search, category, attrib_values, search_in_description=True):
        ...
        res = expression.OR([res, [('product_website_brand_id', 'in', [int(b) for b in request.httprequest.args.getlist('brand',)])],\
    ...
    def _rbt_count(self, search_product, brand_list, tag_list):
        ...
        for prod in search_product:
            if prod.product_website_brand_id and prod.product_website_brand_id in brand_list:
                brand_count[str(prod.product_website_brand_id.id)] += 1
